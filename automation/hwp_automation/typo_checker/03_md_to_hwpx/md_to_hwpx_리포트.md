...
> 작성일: 2026-02-18 16:54 (v0.51 — 표 2 누락 버그 수정 반영)
...
| ----------- | ----------------------------------------------- |
| 입력 파일   | `건의사항_종합표.md`                            |
| 출력 파일   | `hwpx_output/건의사항_종합표_202602181652.hwpx` |
...
| 버전  | 날짜             | 내용                                                    |
| ----- | ---------------- | ------------------------------------------------------- |
| v0.50 | 2026-02-18 16:41 | 초기 버전 — 제목/표/인용문/문단/수평선 변환 지원        |
| v0.51 | 2026-02-18 16:52 | **버그 수정**: 표 2(재료별 상세표)가 누락되던 문제 해결 |

### v0.51 버그 수정 상세

- **증상**: 마크다운에 표가 2개 이상일 때 첫 번째 표만 HWPX에 삽입되고 두 번째 표 이후가 누락됨
- **원인**: `_write_table()` 메서드에서 표 작성 완료 후 `Cancel()` + `MoveDown` 조합이 커서를 표 밖으로 이동시키지 못함
- **수정**: `Cancel()` × 2 + `MoveDocEnd` + `BreakPara()`로 변경하여 문서 끝에서 새 문단 시작
